...
DECLARE
    ...
    C_OP_UPDATE CONSTANT TEXT := 'UPDATE';
BEGIN
    IF TG_OP = C_OP_INSERT THEN
        NEW.created_at := CURRENT_TIMESTAMP;
        NEW.updated_at := CURRENT_TIMESTAMP;
    ELSIF TG_OP = C_OP_UPDATE THEN
...
BEGIN
    IF NEW.status = C_DELETED AND (OLD.status IS NULL OR OLD.status != C_DELETED) THEN
        NEW.deleted_at := CURRENT_TIMESTAMP;
    ELSIF NEW.status != C_DELETED AND OLD.status = C_DELETED THEN
        NEW.deleted_at := NULL;
    END IF;
    RETURN NEW;
END;
...
DECLARE
    ...
    C_DELETED_AT   CONSTANT TEXT := 'deleted_at';
    C_CREATED_BY   CONSTANT TEXT := 'created_by';
    C_UPDATED_BY   CONSTANT TEXT := 'updated_by';

    -- Constants for values
    C_VAL_ACTIVE   CONSTANT TEXT := 'ACTIVE';
    C_VAL_INACTIVE CONSTANT TEXT := 'INACTIVE';
    C_VAL_DELETED  CONSTANT TEXT := 'DELETED';
BEGIN
    ...
    IF NOT EXISTS (SELECT 1
        ...
        EXECUTE v_sql;
        RAISE NOTICE '  ✓ Columna % agregada', C_STATUS;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1
        ...
        EXECUTE v_sql;
        RAISE NOTICE '  ✓ Columna % agregada', C_CREATED_AT;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1
        ...
        EXECUTE v_sql;
        RAISE NOTICE '  ✓ Columna % agregada', C_UPDATED_AT;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1
        ...
        EXECUTE v_sql;
        RAISE NOTICE '  ✓ Columna % agregada', C_DELETED_AT;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1
        ...
        EXECUTE v_sql;
        RAISE NOTICE '  ✓ Columna % agregada', C_CREATED_BY;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1
        ...
        EXECUTE v_sql;
        RAISE NOTICE '  ✓ Columna % agregada', C_UPDATED_BY;
    END IF;
...
DECLARE
    ...
    C_BASE_TABLE  CONSTANT TEXT := 'BASE TABLE';
    C_PG_PATTERN  CONSTANT TEXT := 'pg_%';
    C_SQL_PATTERN CONSTANT TEXT := 'sql_%';
BEGIN
    FOR v_table IN
